...
<div class="main-slider">
    <div class="slider-container">
        <div class="slider-wrapper">
            ...
            <?php
            $slideLink = isset($slide['link_url']) && trim((string) $slide['link_url']) !== '' ? trim($slide['link_url']) : null;
            $slideHoverText = isset($slide['hover_text']) && trim((string) $slide['hover_text']) !== '' ? trim($slide['hover_text']) : null;
            $imgTitle = $slideHoverText ? sanitize($slideHoverText) : '';
            ?>
            <div class="slider-slide <?php echo $i === 0 ? 'active' : ''; ?>">
                <?php if ($slideLink): ?>
                <a href="<?php echo sanitize($slideLink); ?>" target="_blank" rel="noopener noreferrer" class="slider-slide-link" <?php echo $imgTitle ? 'title="' . $imgTitle . '"' : ''; ?>>
                <?php endif; ?>
                <img src="<?php echo normalizeSliderImageUrl($slide['image_url']); ?>" alt="<?php echo sanitize($slide['alt_text'] ?? 'Banner'); ?>" <?php if ($imgTitle): ?>title="<?php echo $imgTitle; ?>"<?php endif; ?> loading="<?php echo $i === 0 ? 'eager' : 'lazy'; ?>" decoding="async">
                <?php if ($slideLink): ?>
                </a>
                <?php endif; ?>
            </div>
