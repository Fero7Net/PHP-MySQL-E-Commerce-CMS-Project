...
function addSliderSlide(PDO $pdo, string $imageUrl, ?string $altText, int $sortOrder, ?string $hoverText = null, ?string $linkUrl = null): bool
{
    $stmt = $pdo->prepare('INSERT INTO slider_slides (sort_order, image_url, alt_text, hover_text, link_url) VALUES (:sort_order, :image_url, :alt_text, :hover_text, :link_url)');
    try {
        return $stmt->execute([
...
function updateSliderSlideMeta(PDO $pdo, int $id, ?string $hoverText, ?string $linkUrl): bool
{
    try {
        $stmt = $pdo->prepare('UPDATE slider_slides SET hover_text = :hover_text, link_url = :link_url WHERE id = :id');
        return $stmt->execute([
            'hover_text' => $hoverText,
            'link_url' => $linkUrl ? trim($linkUrl) : null,
            'id' => $id,
        ]);
    } catch (PDOException $e) {
        if ($e->getCode() === '42S22' || strpos($e->getMessage(), 'Unknown column') !== false) {
